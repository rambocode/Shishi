...
`swift run Shishi` 可启动主界面，但未打包的进程没有系统通知所需的应用身份。启动前检测主 Bundle，跳过系统通知中心初始化；提醒时间照常保存到本地，设置提醒时反馈需启动 `拾事.app` 才能发送。不会误报已调度。使用 `bash scripts/package.sh release` 打包后再启动应用即可使用原有授权与调度流程。
